...
RSpec.describe ControllerAuthentication do
  let(:user_session) { create(:user_session) }
  let(:admin_session) { create(:admin_session) }

  controller(ActionController::Base) do
    include ControllerAuthentication # rubocop:disable RSpec/DescribedClass
  ...
  describe '#after_authentication_url' do
    ...
    context 'when return_to_after_authenticating is not set' do
      it 'returns the dashboard URL' do
        ...
        expect(url).to eq(dashboard_url)
      end
    end
  end
  ...
  describe '#resume_session' do
    ...
    context 'when user is authenticated' do
      ...
      it 'assigns a session' do
        ...
        expect(Current).to have_received(:session=).with(user_session)
      end
    end

    context 'when user is authenticated but session id is invalid' do
      before do
      ...
      it 'does not assign a session and removes session_id from cookie' do
        get :index
  ...
  describe '#start_new_session_for' do
    ...
    it 'creates a new session for the user' do
      expect { controller.send(:start_new_session_for, user) }.to change(UserSession, :count).by(1)
    ...
    it 'sets the Current.session' do
      ...
      expect(Current).to have_received(:session=).with(instance_of(UserSession))
    end

    it 'sets the session cookie' do
      controller.send(:start_new_session_for, user)

      expect(cookies.signed[:session_id]).to eq(user.sessions.last.id)

      cookie_to_set = cookies.instance_variable_get(:@set_cookies)['session_id']
  ...
  describe '#terminate_session' do
    ...
    it 'destroys the current session' do
      ...
      expect(user_session).to be_destroyed
    end
